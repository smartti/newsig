...
class NutritionsModelNutrition extends JModel
{
    ...
    function &getData()
    {
        ...
        if (empty( $this->_data )) {
            $query = "SELECT F.id_familia, V.id_departamento, V.id_provincia, V.id_distrito, D.id_ubigeo, D.tx_centro_poblado, D.id_dg_cate_centro_poblado, D.id_direccion_familia,  
                      D.tx_nucleo_urbano, D.id_dg_cate_nucleo_urbano, D.id_dg_via, D.tx_nombre_via, D.un_puerta, D.un_piso, D.tx_zona, D.un_interior, D.tx_manzana, 
                      D.tx_manzana_nucleo, D.un_vivienda, D.tx_lote, D.un_km, F.tx_apellidos, F.id_dg_tipo_vivienda, F.tx_observacion_vivienda, 
                      F.id_dg_material_techo, F.tx_observacion_techo, F.id_dg_material_paredes, F.tx_observacion_paredes, F.id_dg_material_piso, F.tx_observacion_piso, 
                      ...
                      F.id_dg_resultado, F.id_dg_resultado2, F.id_dg_resultado3, F.un_habitaciones, F.in_cria_animales, D.un_hogares, D.un_hogar, CONCAT_WS(' ',P.tx_apellido_paterno, P.tx_apellido_materno, P.tx_nombres) AS encuestador_name 
                      FROM familia as F INNER JOIN direccion_familia D ON (F.id_familia = D.id_familia) 
                      INNER JOIN persona AS P ON (F.id_persona_encuestador = P.id_entidad)
    ...
    public function getCentros() {
        $query = "SELECT id_detalle_general as value, nu_categoria as text FROM detalle_general WHERE id_general = '1'";
        $this->_db->setQuery($query);
    ...
    public function getNucleos() {
        $query = "SELECT id_detalle_general as value, nu_categoria as text FROM detalle_general WHERE id_general = '2'";
        $this->_db->setQuery($query);
